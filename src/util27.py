...
'''
# write oracle feature
format: 
    oracleid (inside), feature
'''
def writeOracles(output_file, u_features):
    # weak oracle
    exist = True
    if u_features is None: exist = False
    with open(output_file + 'oracle.csv', mode='w') as oracle_file:
        writer = csv.writer(oracle_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        # writer.writerow(['oracle_id', 'm1', 'm2', 'm3', 'b1', 'b2'])
        if exist:
            for oi in range(len(u_features)):
                oi_feat = [oi]
                oi_feat.extend(u_features[oi])
                writer.writerow(oi_feat)
        else:
            for oi in range(len(u_features)):
                writer.writerow([oi, 1])  # 1 for uniform feature
    oracle_file.close()

'''
Write instance feature
format:
    instanceid (inside), feature
'''
def writeInstances(output_file, v_features):
    # instances
    exist = True
    if v_features is None: exist = False
    with open(output_file+'instance.csv', mode='w') as instance_file:
        writer = csv.writer(instance_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        #writer.writerow(['instance_id', 'features'])
        if exist:
            for id in range(v_features.shape[0]):
                i_feat = [id]
                tfidf_feat = v_features[id].toarray()[0]
                i_feat.extend(tfidf_feat)
                writer.writerow(i_feat)
        else:
            for id in range(v_features.shape[0]):
                writer.writerow([id, 1])  # 1 for uniform feature
    instance_file.close()

'''
write observed responses
input: response (inside):
output format:
    oracleid (inside), instance id (inside), label (0/1)
'''
def writeObserveResponse(output_file, responses):
    with open(output_file + 'response.csv', mode='w') as response_file:
        writer = csv.writer(response_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        #writer.writerow(['oracle_id', 'user_id', 'response'])
        for id in responses:
            for oi in responses[id]:
                writer.writerow([oi, id, responses[id][oi]])
    response_file.close()

'''
write label
input: labels (original)
output: 
'''
def writeLabels(output_file, labels):
    with open(output_file + 'label.csv', mode='w') as label_file:
        writer = csv.writer(label_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        for id in range(len(labels)):
            writer.writerow([id, labels[id]])
    label_file.close()

def writeAllData(output_file, u_features, v_features, responses, labels):
    writeOracles(output_file, u_features)
    writeInstances(output_file, v_features)
    writeObserveResponse(output_file, responses)
    writeLabels(output_file, labels)
...
if __name__ == '__main__':
    ...
    '''
    lb0_instance_idx = [id for id in range(len(true_labels)) if true_labels[id] == 0]
    filtered_instance = random.sample(lb0_instance_idx, label_num[1])
    lb1_instance_idx = [id for id in range(len(true_labels)) if true_labels[id] == 1]

    filtered_instance.extend(lb1_instance_idx)
    filtered_instance.sort()

    # oracles
    filtered_oracle = {}
    for id in filtered_instance:
        for oi in responses[id]:
            filtered_oracle[oi] = 1
    filtered_oracle = list(filtered_oracle.keys())
    filtered_oracle.sort()

    oracle_idx = {filtered_oracle[i]: i for i in range(len(filtered_oracle))}
    instance_idx = {filtered_instance[i]:i for i in range(len(filtered_instance))}

    u_features = u_features[filtered_oracle]
    v_features = v_features[filtered_instance]
    responses = {id: responses[id] for id in filtered_instance}
    responses = Utils.responseOut2In(responses, oracle_idx, instance_idx)
    true_labels = true_labels[filtered_instance]
    '''
    # sample balanced dataset]
    ...
    writeAllData("../../../../MatrixCompletion/venv/src/gc_mc_master/gcmc/data/dec/" + dataName + "_", u_features,
                 v_features, responses, true_labels)
